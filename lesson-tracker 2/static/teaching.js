...
function navigationItems(){
 ...
 return [...(role==='owner'?[['accounting','账目总结','grid'],['administrators','管理员','users']]:[]),['schedule',['owner','admin'].includes(role)?'排课日历':role==='teacher'?'我的课表':'孩子的课表','clock'],['feedback','课后记录','book'],['overview',['owner','admin'].includes(role)?'学生与课时':role==='teacher'?'我的学生':'剩余课时','grid'],['records','课时明细','book'],...(['owner','admin'].includes(role)?[['teachers','老师管理','users']]:[])];
}
function dashboard(){
...
function renderOverview(){
 const students=state.students,total=students.reduce((n,s)=>n+s.balance,0),staff=isStaff();
 $('#surface').innerHTML=`<section class="stats"><div class="stat featured"><div class="stat-label">${staff?'学生剩余课时合计':'孩子剩余课时合计'}${icon('clock')}</div><div class="stat-value">${fmt(total)}<small>课时</small></div><div class="stat-note">以实际课时账本为准</div></div><div class="stat"><div class="stat-label">${staff?'关联学生':'关联孩子'}${icon('users')}</div><div class="stat-value">${students.length}<small>位</small></div><div class="stat-note">${staff?'可在课表中查看课程安排':'仅显示自己孩子的信息'}</div></div><div class="stat"><div class="stat-label">课时较少</div><div class="stat-value">${students.filter(s=>s.balance<=200).length}<small>位</small></div><div class="stat-note">剩余不超过 2 课时</div></div></section>${staff?'<div class="section-head"><h2>学生课时</h2><input id="search" class="search" type="search" placeholder="搜索学生、课程或手机号" aria-label="搜索学生"></div><div id="student-list"></div>':parentCards()}<section class="recent"><div class="section-head"><h2>最近课时记录</h2><button class="action" id="all-records">全部记录 →</button></div><div id="records-list"></div></section>`;
...
function timeCalendar(rows){
 ...
 return `<td class="${day===state.today?'calendar-today':''}">${list.map(lessonCard).join('')}${isStaff()?`<button class="calendar-slot ${list.length?'slot-add':''}" data-slot="${slot}" aria-label="${day} ${hour} · 新增排课">${list.length?'+':'<span>暂无课程</span><small>＋ 排课</small>'}</button>`:list.length?'':'<span class="calendar-empty">暂无课程</span>'}</td>`;
 }).join('')}</tr>`;
...
function renderTeachers(){
 $('#surface').innerHTML=`<section class="teacher-grid">${state.teachers.map(t=>{const own=state.lessons.filter(l=>l.teacher_id===t.id),upcoming=own.filter(l=>l.status==='scheduled').length;return `<article class="teacher-card"><div class="child-top"><span class="student-icon">${userHTML(t.name.slice(0,1))}</span><div><h2>${userHTML(t.name)}</h2><p>${userHTML(t.subject)} · ${t.user_id===state.user.id?'管理员 / 任课老师':'任课老师'}</p></div></div><p class="teacher-phone">${userHTML(t.phone)}</p><div class="teacher-bottom"><span>待完成 <strong>${upcoming}</strong> 节</span><button class="action" data-teacher-calendar="${t.id}">查看课表 →</button></div></article>`;}).join('')}</section><p class="footnote">老师使用独立手机号登录，仅可查看自己的课表、关联学生及自己填写的课后记录。</p>`;
...
function teacherModal(){modal('添加任课老师',`<label class="field">老师姓名<input name="name" required maxlength="40" placeholder="例如：Emma 老师"></label><label class="field">教学科目<input name="subject" required maxlength="60" value="数学"></label><label class="field">登录手机号<input name="phone" type="tel" required maxlength="24" placeholder="例如 +61 412 345 678"></label><label class="field">初始密码<input name="password" type="password" autocomplete="new-password" minlength="10" maxlength="128" required><small>老师可以登录后修改。请使用独立手机号，不与家长账号共用。</small></label>`,'开通老师账号',async data=>{await api('teachers',data);toast('老师账号已开通，可以开始排课。');});}
